(function () {
  ...
  function assignmentHistory(ticket) {
    ...
    return `<ul class="assignment-history">${ticket.assignments.slice().reverse().map(item => `<li>${assignmentBadge(item.state)}<span><strong>${esc(item.unitLabel)}</strong> · ${esc(item.officerName || item.officerEmail)}</span><small>Assigned ${esc(date(item.assignedAt))}${item.openedAt ? ` · Opened ${esc(date(item.openedAt))}` : ''}${item.resolvedAt ? ` · Resolved ${esc(date(item.resolvedAt))}` : ''}${item.supersededAt ? ` · Redirected ${esc(date(item.supersededAt))}` : ''}${item.decisionEvidence?.length ? ` · ${esc(item.decisionEvidence.length)} decision evidence file${item.decisionEvidence.length===1?'':'s'}` : ''}</small>${item.resolutionNote ? `<details class="assignment-history-narrative"><summary>View recorded narrative</summary><p>${esc(item.resolutionNote)}</p></details>` : ''}</li>`).join('')}</ul>`;
  }
  function internalFeedback(ticket) {
    const items = ticket.internalFeedback || [];
    if (!items.length) return '<p class="admin-ticket-empty">No private feedback has been sent by assigned staff.</p>';
    return `<ul class="case-history">${items.slice().reverse().map(item => `<li><strong>${esc(item.fromName || item.fromEmail || 'Assigned staff')}</strong><span>${esc(date(item.at))} · ${esc(item.unitLabel || '')}</span><p>${esc(item.message)}</p></li>`).join('')}</ul>`;
  }
  ...
  function ticketCard(ticket) {
    ...
    return `<details class="admin-ticket" data-id="${esc(ticket.id)}">
      ...
      <div class="ticket-body">
        ...
        ${identity.role === 'administrator' && ticket.internalFeedback?.length ? `<section class="feedback-detail"><h4>Private feedback from assigned staff</h4><p>Visible to authorised unit administrators only. Students do not see these messages.</p>${internalFeedback(ticket)}</section>` : ''}
