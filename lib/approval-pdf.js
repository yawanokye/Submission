...
function createApprovalPdf({signaturePath,data={}}={}) {
  ...
  const info=add(`<< /Title (${pdfEscape(`Approved claim ${data.reference||''}`)}) /Author (${pdfEscape(data.hodName||'University of Cape Coast')}) /Subject (Electronic approval for payment) /Creator (CoDE eServices Portal) >>`);
