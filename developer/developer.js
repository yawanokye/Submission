...
async function loadCentres(){const data=await getJson('/api/developer/study-centres'),catalogue=data.departments||{};document.getElementById('centreCount').textContent=data.total??Object.values(catalogue).reduce((n,list)=>n+(Array.isArray(list)?list.length:0),0);document.getElementById('centreTags').innerHTML=Object.keys(deptLabels).map(slug=>{const list=Array.isArray(catalogue[slug])?catalogue[slug]:[];return `<section class="centre-group"><h4>${esc(deptLabels[slug])}</h4>${list.length?list.map(c=>`<span class="centre-tag">${esc(c)}</span>`).join(''):'<div class="centre-empty">No centres published for this department.</div>'}</section>`}).join('');}
