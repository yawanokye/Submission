...
async function main(){
  const storage=await fsp.mkdtemp(path.join(os.tmpdir(),'ucc-code-eservices-v48-')),dataDir=path.join(storage,'data');await fsp.mkdir(dataDir,{recursive:true});await fsp.mkdir(path.join(storage,'files'),{recursive:true});
  ...
  const fieldRecord={id:'field-category-fixture',reference:'FIELD-CATEGORY-001',portalType:'field-experience',department:'education',departmentName:'Department of Education Programmes',submittedAt:'2026-09-24T08:30:00.000Z',reviewedAt:'2026-09-24T09:30:00.000Z',reviewedBy:'Fixture reviewer',reviewStatus:'returned',fullName:'Dr Field Supervisor',email:'field.supervisor@example.edu',phone:'0240000001',studyCentre:'',studyCentres:[],studentStream:'non-residential',projectStream:'non-residential',classificationVersion:0,assessmentType:'micro-teaching',assessmentLabel:'Micro-Teaching',groupCount:'1',claimedCandidateCount:1,scoreSheet:{scoreHeaders:['SCORE'],rows:[{originalSn:'1',name:'Field Student',registrationNo:'BEP/CC/02/001',scoreValues:['79'],scoreHeaders:['SCORE']}]},fieldScoreReviewExcludedRows:[],files:{}};
  ...
  await Promise.all([fsp.writeFile(path.join(dataDir,'submissions.json'),JSON.stringify([record,fieldRecord],null,2)),fsp.writeFile(path.join(dataDir,'admin-users.json'),JSON.stringify([account],null,2)),fsp.writeFile(path.join(dataDir,'study-centres.json'),JSON.stringify({version:3,departments:{education:['Cape Coast','Non-Residential']}},null,2))]);
  ...
  try{
    ...
    const projectBack=await json('/api/admin/education/submissions/project-batch-fixture/classification',{method:'PATCH',headers:{...headers,'content-type':'application/json'},body:JSON.stringify({studentStream:'distance',studyCentres:['Cape Coast'],reason:'Confirmed that this record belongs to the Distance cohort.'})});assert.equal(projectBack.toStream,'distance');assert.deepEqual(projectBack.studyCentres,['Cape Coast']);
    let submissions=await json('/api/admin/education/submissions',{headers}),project=submissions.find(item=>item.id==='project-batch-fixture');assert.equal(project.studentStream,'distance');assert.equal(project.studyCentre,'Cape Coast');assert.equal(project.classificationVersion,2);
    const fieldToDistance=await json('/api/admin/education/submissions/field-category-fixture/classification',{method:'PATCH',headers:{...headers,'content-type':'application/json'},body:JSON.stringify({studentStream:'distance',studyCentres:['Cape Coast'],reason:'The returned Field Experience record belongs to a Distance study centre.'})});assert.equal(fieldToDistance.toStream,'distance');assert.deepEqual(fieldToDistance.studyCentres,['Cape Coast']);
    const fieldBack=await json('/api/admin/education/submissions/field-category-fixture/classification',{method:'PATCH',headers:{...headers,'content-type':'application/json'},body:JSON.stringify({studentStream:'non-residential',studyCentres:[],reason:'Administrative review confirmed the student is Non-Residential.'})});assert.equal(fieldBack.toStream,'non-residential');assert.deepEqual(fieldBack.studyCentres,[]);
    submissions=await json('/api/admin/education/submissions',{headers});const field=submissions.find(item=>item.id==='field-category-fixture');assert.equal(field.studentStream,'non-residential');assert.equal(field.studyCentre,'Non-Residential');assert.equal(field.classificationVersion,2);assert.equal(field.fieldReviewStatus,'returned');
    list=await json('/api/admin/education/score-batches',{headers});assert.equal(list.unbatched['project-work:distance'],1,'a record moved back to Distance must be available for a new batch');
    console.log('v48 processing batches and two-way category movement verified');
